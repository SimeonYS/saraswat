...
class SaraswatPipeline:

    # Database setup
    conn = sqlite3.connect('saraswat.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `saraswat`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM saraswat WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO saraswat (date, title, link, content)"
